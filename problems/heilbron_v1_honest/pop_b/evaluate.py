"""Adversarial evaluate.py for Pop B (Improver) — v1 linear improvement scoring.

Receives:
    opponent_results: list of (11, 2) np.ndarray  (point configs from Pop A)
    program_output:   callable improve(points) -> improved_points

Per-opponent score = min(max(raw_delta, 0) / Q_MAX, 1.0). Regressions clamp to 0;
full Q_MAX gap closure → 1.0. ConfigurableAggregator composes per_opp_metrics into
program-level metrics via heilbron_improver.yaml. v1 hard-floor scoring restored
verbatim for honest v1 reproduction (heilbron/v1-honest-repro). The continuous
tanh variant of D-side scoring lives in problems/heilbron_repro_v1/pop_b/ (it has
since been reverted upstream too — kept here as the explicit pinned v1 contract).
"""
