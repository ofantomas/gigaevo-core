"""Adversarial evaluate.py for Pop A (Constructor) — v1 binary resistance.
...
Intrinsic metrics (quality, actual_fitness) are candidate-level. Per-opponent
resistance is BINARY (1.0 if opponent failed to improve, 0.0 if it improved by any
positive delta) and is reduced by the aggregator via heilbron_constructor.yaml.
This is the v1 hard-floor scoring restored for honest reproduction of the v1 SOTA
(heilbron/v1-honest-repro). Continuous-resistance variant lives in
problems/heilbron_repro_v1/pop_a/.
"""
...
def evaluate(opponent_results: list, program_output: object):
    ...
    for improve_fn in opponent_results:
        ...
        try:
            ...
            # v1 binary hard-floor: 1.0 if opponent failed to improve, else 0.0.
            resistance_score = float(delta <= 0.0)
