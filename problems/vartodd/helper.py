...
def _to_erank_schedule(x: Any) -> "RankSchedule":
    ...
    if isinstance(x, zip):
        x = [obj for obj in x]
    if _is_rank_list(x):
        return RankSchedule.from_any(
            [(rank, _to_exploration_score(el)) for (rank, el) in x]
        )
...
def _to_frank_schedule(x: Any) -> "RankSchedule":
    ...
    if _is_rank_list(x):
        return RankSchedule.from_any(
            [(rank, _to_finalization_score(el)) for (rank, el) in x]
        )
...
def dao_rank_to_str(daos: list[Dao], ranks: list[int]):
    dict = {}
    dict["num_samples"] = int_rank_shedule_to_str(
        [dao.mode.num_samples for dao in daos], ranks
    )
    dict["top_pool"] = int_rank_shedule_to_str(
        [dao.mode.top_pool for dao in daos], ranks
    )
    ...
    dict["pool_scores"] = score_rank_shedule_to_str(
        [dao.mode.pool_scores for dao in daos], ranks
    )
    dict["final_scores"] = score_rank_shedule_to_str(
        [dao.mode.final_scores for dao in daos], ranks
    )
    dict["max_z_to_research"] = int_rank_shedule_to_str(
        [dao.mode.max_z_to_research for dao in daos], ranks
    )
    dict["gen_part"] = float_rank_shedule_to_str(
        [dao.mode.gen_part for dao in daos], ranks
    )
...
def summarize_path_backups(
    root_dir: str = "data/path_backups", top_k: int = 10, init_word: str = "init"
) -> str:
    ...
    total = len(records)
    return f"best_paths={best_paths}\ncount_total={total}"
...
class BaseEvaluator:
    ...
    best_paths: list[Path] = field(default_factory=list)
    best_report: str
    best_pcfg: str
    best_eval: int = 0
    total_eval: int = 0
    ...
    def __init__(
        ...
        if mat is None and path_name == "init":
            ...
            if init_rank_thr is None:
                raise ValueError(
                    "init_rank_thr must be provided when path_name is not 'init'"
                )
    ...
    def run(self, params, seeds, max_workers=2):
        if len(params) != len(self.active_params):
            raise RuntimeError(
                f"Num of params {len(params)} is not equal to the num of active params {len(self.active_params)}"
            )
        self.insert(params)
        self.reinit()
        ...
        for _, node, counters, seed in results:
            ...
            if rank < self._best_rank:
                ...
                self.best_paths = [
                    self.current_path.branch_path(node, self.dao, self.x0)
                ]
            if rank == self._best_rank:
    ...
    def get_best(self):
        ...
        return (
            ...
            + str(
                dao_rank_to_str(
                    self.best_paths[0].daos, self.best_paths[0].ranks_thr + [0]
                )
            )
            + "\nsearch_stat:\n"
    ...
    def _pick_path_for_save(self) -> Path:
        ...
        best_rank = min(
            p.final_node.state.rows for p in self.best_paths if p.final_node is not None
        )
        candidates = [
            p
            for p in self.best_paths
            if p.final_node is not None and p.final_node.state.rows == best_rank
        ]
    ...
    def _auto_hashed_name(self, name: str, path: Path) -> str:
        ...
        mid_rank = (
            int(self.init_rank_thr) if self.init_rank_thr is not None else init_rank
        )
