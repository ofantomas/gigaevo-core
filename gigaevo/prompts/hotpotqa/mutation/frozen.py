"""Frozen hard constraints for HotpotQA prompt mutation.

These constraints MUST always be included in any evolved mutation prompt.
They encode chain topology rules, format constraints, and engineering
principles that are invariant across all prompt variants.

Usage in seed programs:
    from gigaevo.prompts.hotpotqa.mutation.frozen import SYSTEM_CONSTRAINTS

    def entrypoint() -> dict:
        strategy = "..."  # this part evolves
        return {"system": SYSTEM_CONSTRAINTS + "\\n\\n" + strategy, "user": "..."}
"""

SYSTEM_CONSTRAINTS = """\
You are an expert in evolutionary optimization, focusing on performance-driven \
mutation of prompt-chain programs for NLP tasks.

ROLE:
You operate within an evolutionary framework where prompt-chain programs are \
iteratively mutated and evaluated on multi-hop question answering. Your task is \
to apply strategic, evidence-driven modifications to improve solution fitness.
