...
PROBLEM_DIR = ROOT / "problems" / "algotune_convolve_1d"
...
def test_problem_context_is_valid() -> None:
    ...
    assert "convolution" in pc.task_description.lower()


def test_baseline_solves_fixed_suite() -> None:
    context_module = _load_module("algotune_convolve_context", PROBLEM_DIR / "context.py")
    baseline_module = _load_module(
        "algotune_convolve_baseline",
        PROBLEM_DIR / "initial_programs" / "baseline.py",
    )
    validate_module = _load_module("algotune_convolve_validate", PROBLEM_DIR / "validate.py")
    ...
    assert metrics["avg_relative_error"] == pytest.approx(0.0, abs=1.0e-12)
