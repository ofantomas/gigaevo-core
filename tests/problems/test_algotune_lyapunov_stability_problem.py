...
PROBLEM_DIR = ROOT / "problems" / "algotune_lyapunov_stability"
...
def test_problem_context_is_valid() -> None:
    ...
    assert "lyapunov" in pc.task_description.lower()


def test_baseline_solves_fixed_suite() -> None:
    context_module = _load_module(
        "algotune_lyapunov_context", PROBLEM_DIR / "context.py"
    )
    baseline_module = _load_module(
        "algotune_lyapunov_baseline",
        PROBLEM_DIR / "initial_programs" / "baseline.py",
    )
    validate_module = _load_module(
        "algotune_lyapunov_validate", PROBLEM_DIR / "validate.py"
    )
