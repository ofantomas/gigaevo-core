...
class TestCardType:
    ...
    def test_pydantic_memory_card(self):
        """Bug #3 (PR #161): _card_type must accept Pydantic MemoryCard."""
        card = MemoryCard(id="idea-1", category="general", description="test")
        assert _card_type(card) == "ideas"

    def test_pydantic_program_card(self):
        """Bug #3 (PR #161): _card_type must accept Pydantic ProgramCard."""
        card = ProgramCard(id="prog-1", program_id="p1", category="program")
        assert _card_type(card) == "programs"
