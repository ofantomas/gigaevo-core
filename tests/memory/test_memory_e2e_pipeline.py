...
class TestPydanticAttributeAccess:
    """Verify that every attribute accessed on Pydantic models in main() works.

    The main() function accesses .description on objects returned by
    memory.get_card() (which returns AnyCard | None). These tests ensure
    attribute access never calls .get() on Pydantic models.
    """

    def test_memory_card_description_access(self):
        """MemoryCard.description is accessible as attribute (not .get())."""
        card = MemoryCard(
            id="idea-1", category="general", description="Test description"
        )
        # This is what main() line 656 does after the fix
        desc = card.description or ""
        assert desc == "Test description"
        assert desc[:110] == "Test description"

    def test_program_card_description_access(self):
        """ProgramCard.description is accessible as attribute."""
        card = ProgramCard(
            id="prog-1",
            program_id="p1",
            category="program",
        ...
        desc = card.description or ""
        assert desc == "Program description"

    def test_memory_card_no_get_method(self):
        """Pydantic models do NOT have .get() — confirms the bug pattern."""
        card = MemoryCard(id="idea-1", category="general", description="test")
        assert not hasattr(card, "get"), "Pydantic models should not have .get()"

    def test_full_main_loop_simulation_with_description(self, tmp_path):
        """Simulate the FULL main() loop including description access.

        This covers the exact code path that crashed at line 656:
        1. load_memory_cards() → Pydantic models
        2. _card_type(card) → classify
        3. card.description → attribute access (NOT .get())
        """
        banks_path = tmp_path / "banks.json"
        _write_json(
            banks_path,
            [
                {
                    "active_bank": [
                        make_ideas_tracker_card(
                            "idea-1", "Chunked retrieval", has_version_history=True
                        ),
                    ]
                }
            ],
        ...
        # Simulate FULL main() loop including description access
        for card in cards:
            card_type = _card_type(card)
            assert card_type in ("ideas", "programs")

            # This is what main() does after save_card + get_card
            # The returned card is the same Pydantic type
            if isinstance(card, (MemoryCard, ProgramCard)):
                stored_desc = card.description or ""
            elif isinstance(card, dict):
                stored_desc = card.get("description", "")
            else:
                stored_desc = ""

            # Must not crash, must produce a string
            assert isinstance(stored_desc, str)
            assert len(stored_desc[:110]) <= 110
