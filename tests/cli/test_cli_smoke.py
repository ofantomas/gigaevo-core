...
class TestCliSmoke:
    ...
    def test_subcommand_help_is_nonempty(self, cmd_name: str) -> None:
        ...
        assert result.exit_code == 0, f"{cmd_name} --help exited {result.exit_code}: {result.output}"
        assert len(result.output) > 100, f"{cmd_name} --help output too short ({len(result.output)} chars)"
        assert "No help" not in result.output, f"{cmd_name} --help contains 'No help' — missing option help="

    def test_group_commands_have_children(self) -> None:
        """Group commands (plot, manifest, flush) list their children."""
        groups = {"plot": ["comparison", "trajectory", "arms-race"],
                  "manifest": ["get", "update", "gate", "pr-description"],
                  "flush": ["dry-run"]}  # flush dry-run is a group itself
        for group_name, expected_children in groups.items():
            ...
            for child in expected_children:
                assert child in result.output, f"{child} not listed in {group_name} --help"
    ...
    def test_group_leaf_help_is_nonempty(self, group: str, children: list[str]) -> None:
        ...
        for child in children:
            runner = CliRunner()
            result = runner.invoke(main, [group, child, "--help"], catch_exceptions=False)
            assert result.exit_code == 0, f"{group} {child} --help exited {result.exit_code}: {result.output}"
            assert len(result.output) > 50, f"{group} {child} --help too short ({len(result.output)} chars)"
            assert "No help" not in result.output, f"{group} {child} --help contains 'No help'"
    ...
    def test_root_help_shows_examples_and_flag_ordering(self) -> None:
        ...
        assert "Argument order" in result.output, "Root --help should mention flag ordering"
        assert "-e" in result.output and "-r" in result.output, "Global flags should be documented"
        ...
        for group in ["plot", "manifest"]:
            ...
            for line in result.output.split('\n'):
                ...
                if line.startswith("  ") and not line.startswith("    "):
                    ...
                    if parts:
                        child_name = parts[0]
                        child_result = runner.invoke(main, [group, child_name, "--help"], catch_exceptions=False)
